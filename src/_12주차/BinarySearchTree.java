...
public class BinarySearchTree {
	...
	private void setLeft(BinarySearchTree T) {
		_left = T;
	}
	...
	private void setRight(BinarySearchTree T) {
		_right = T;
	}
	...
	public boolean insert(int key) {
		if (this._key == null) {
			...
			if (flag == 0)
				...
				if (_left == null)
					_left = new BinarySearchTree();
				return _left.insert(key);
			} else {
				if (_right == null)
					_right = new BinarySearchTree();
				return _right.insert(key);
			}
		}
	}

	public int getKey(BinarySearchTree bst) {
		return ((Node) bst.deleteMinimum(bst))._key;
	}
		...
		if (flag == 0) {
			...
			if (_left == null && _right == null) {
				...
				if (_right.treeSize() == 1) {
					...
					_key = _right.deleteMinimum(_right);
			}
